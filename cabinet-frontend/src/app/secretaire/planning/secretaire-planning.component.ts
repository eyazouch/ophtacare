    ...
    <div class="card">
      ...
      <div class="table-container">
        <table *ngIf="planning.length > 0">
          <thead>
            <tr><th>Heure</th><th>Patient</th><th>Motif</th><th>Durée</th><th>Statut</th><th>Urgent</th><th>Actions</th></tr>
          </thead>
          <tbody>
            <tr *ngFor="let rdv of planning" [style.background]="rdv.urgent ? '#fef2f2' : ''">
              ...
              <td style="display:flex; gap:10px; align-items:center; justify-content:flex-start;">
                <button class="btn btn-primary btn-sm" (click)="modifierRdv(rdv)">
                    Modifier
                </button>
...
export class SecretairePlanningComponent implements OnInit {
  selectedDate = '';
  planning: RendezVous[] = [];
  ...
  getBadge(statut: string): string {
   ...
   modifierRdv(rdv: RendezVous): void {
    this.router.navigate(['/secretaire/rdv/modifier', rdv.id]);
   }
